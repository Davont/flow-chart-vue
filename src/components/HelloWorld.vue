...
const option = {
  ...
  render: (container, data) => {
    ...
    // let node = `<div>22<div>`;
    // let node = "<div className={`fc-node-example ${nodeClass}`}>{id}</div>";
...
onMounted(() => {
  let chartIns = new HuiCharts();
    ...
    console.log(chartContainerDom);
    
    chartIns.init(chartContainerDom);
    let chartOption = option;
    chartIns.setSimpleOption(FlowChart, chartOption);
    chartIns.render();
})
